...
const ArticleCard = ({ article }) => {
    // 🔹 Hooklar har doim komponentning eng boshida
    const { user } = useSelector((state) => state.auth);
    const [favorited, setFavorited] = useState(article?.favorited || false);
    const [following, setFollowing] = useState(article?.author?.following || false);

    // 🔹 Hooklardan keyin tekshiruv
    if (!article) {
        return <p>Loading...</p>;
    }
        ...
        <div className="card border-0 shadow-lg rounded-4 overflow-hidden mb-4">
            <div className="card-header bg-white border-0 pb-0">
                <div className="d-flex justify-content-between align-items-start">
                    <div className="d-flex align-items-center">
                        <img
                            src={article.author?.image || "https://via.placeholder.com/56"}
                            alt={authorName}
                        ...
                        />
                        <div>
                            <h6 className="mb-0 fw-semibold text-capitalize">{authorName}</h6>
                            <small
                                className="text-muted d-block trimText"
                                style={{
                                    maxHeight: "2.5rem",
                                    overflow: "hidden",
                                }}
            ...
            <div className="card-body d-flex flex-column justify-content-between">
                <div className="d-flex flex-column justify-content-start">
                    ...
                    <p className="text-body-secondary mb-4 trimText">{article.body}</p>
                </div>
